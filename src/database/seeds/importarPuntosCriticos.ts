  ...
  try {
    ...
    for (const dato of response.data) {
      try {
        ...
        // Calcular número aproximado de accidentes basado en el score
        // Si no hay dato directo, estimamos basándonos en fallecidos y heridos
        const numeroAccidentes = fallecidos + heridos > 0
          ? Math.ceil((fallecidos * 3 + heridos) / 2)
          : Math.ceil(gizscore / 10) || 1;

        // Calcular nivel de peligrosidad usando el gizscore
...
// Calcular nivel de peligrosidad basado en gizscore y métricas
